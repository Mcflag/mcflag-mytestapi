 ...
 * @author Martin Appl
 * 
 *         Endless loop with items filling from adapter. Currently only
 *         horizontal orientation is implemented View recycling in adapter is
 *         supported. You are encouraged to recycle view in adapter if possible
 * 
 */
public class EndlessLoopAdapterContainer extends AdapterView<Adapter> {
	...
	/**
	 * If there is not enough items to fill adapter, this value is set to true
	 * and scrolling is disabled. Since all items from adapter are on screen
	 */
	protected boolean isSrollingDisabled = false;

	/**
	 * Whether content should be repeated when there is not enough items to fill
	 * container
	 */
	protected boolean shouldRepeat = true;

	/**
	 * Position to scroll adapter only if is in endless mode. This is done after
	 * layout if we find out we are endless, we must relayout
	 */
	...
	/**
	 * Params describing position of child view in container in HORIZONTAL mode
	 * TOP,CENTER,BOTTOM are active in VERTICAL mode LEFT,CENTER,RIGHT are
	 * active
	 * 
	 * @author Martin Appl
	 * 
	...
	/**
	 * Return offset by which is edge off first item moved off screen. You can
	 * persist it and insert to setFirstItemOffset() to restore exact scroll
	 * position
	 * 
	 * @return offset of first item, or 0 if there is not enough items to fill
	 *         container and scrolling is disabled
	 */
	...
	/**
	 * Negative number. Offset by which is left edge of first item moved off
	 * screen.
	 * 
	 * @param offset
	 */
	...
	/**
	 * Position index must be in range of adapter values (0 - getCount()-1) or
	 * -1 to unselect
	 */
	@Override
	...
	/**
	 * Method for actualizing content after data change in adapter. It is
	 * expected container was emptied before
	 * 
	...
	//
	// specW = getChildMeasureSpec(MeasureSpec.makeMeasureSpec(getWidth(),
	// MeasureSpec.UNSPECIFIED), 0, params.width);
	// specH = getChildMeasureSpec(MeasureSpec.makeMeasureSpec(getHeight(),
	...
	// //put measured values into layout params from where they will be used in
	// layout.
	// //Use measured values only if exact values was not specified in layout
	// params.
	...
	protected void refillLeft() {
		if (!shouldRepeat && isSrollingDisabled)
			return; // prevent next layout calls to override first init to
					// scrolling disabled by falling to this branch
		if (getChildCount() == 0)
			return;
	...
	// */
	// protected void refillLeft(){
	// if(!shouldRepeat && isSrollingDisabled) return; //prevent next layout
	// calls to override override first init to scrolling disabled by falling to
	// this branch
	// final int leftScreenEdge = getScrollX();
	//
	// View child = getChildAt(0);
	...
	//
	// child = mAdapter.getView(mFirstItemPosition,
	...
	protected void removeNonVisibleViews() {
		...
		while (firstChild != null
				&& firstChild.getRight() + ((LoopLayoutParams) firstChild.getLayoutParams()).rightMargin < leftScreenEdge) {
		...
		while (lastChild != null && firstChild != null
				&& lastChild.getLeft() - ((LoopLayoutParams) firstChild.getLayoutParams()).leftMargin > rightScreenEdge) {
			// if selected view is going off screen, remove selected state
	 ...
	 * @return child which was actually added to container, subclasses can
	 *         override to introduce frame views
	 */
	 ...
	 * 
	 * @param left
	 *            positon for left edge in parent container
	 * @param lp
	 *            layout params
	 * @return new left
	 */
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			// usualy propagates to on cancel
			if (mHandleSelectionOnActionUp && mTouchState == TOUCH_STATE_RESTING) {
				final float d = ToolBox.getLineLength(mDown.x, mDown.y, x, y);
		...
		case MotionEvent.ACTION_CANCEL:
			...
			// events will propagate back here and on intercept touch event wont
			// be called again
			// instead we get cancel here, since we stated we shouldn't
			// intercept events and propagate them to children
			// but events propagated back here, because no child was interested
			// if(!mInterceptTouchEvents && mHandleSelectionOnActionUp &&
			// mTouchState == TOUCH_STATE_RESTING){
			// handleClick(mDown);
			// mHandleSelectionOnActionUp = false;
			// }
	...
	/**
	 * Check if list of weak references has any view still in memory to offer
	 * for recyclation
	 * 
	 * @return cached view
	 */
	...
	/**
	 * Only set value for selection position field, no gui updates are done for
	 * setting selection with gui updates and callback calls use setSelection
	 * 
	 * @param position
	 */
