 ...
 * @author Martin Appl
 * 
 *         Horizontally scrollable container with boundaries on the ends, which
 *         places Views on coordinates specified by tile objects. Data binding
 *         is specified by adapter interface. Use abstract adapter which has
 *         already implemented algorithms for searching views in requested
 *         ranges. You only need to implement getViewForTile method where you
 *         map Tile objects from dataset to corresponding View objects, which
 *         get displayed. Position on screen is described by LayoutParams
 *         object. Method getLayoutParamsForTile helps generate layout params
 *         from data objects. If you don't set Layout params in getViewForTile,
 *         this methods is called automatically afterwards.
 * 
...
public class BasicContentBand extends ViewGroup {
	...
	/**
	 * In this mode we have pixel size of DSP specified, if dspHeight is bigger
	 * than window, content band can be scrolled vertically.
	 */
	public static final int GRID_MODE_FIXED_SIZE = 0;
	/**
	 * In this mode is pixel size of DSP calculated dynamically, based on widget
	 * height in pixels and value of dspHeight which is fixed and taken from
	 * adapters getBottom method
	 */
	...
	// for(int j=getChildCount()-1; j >= 0; j--){ //start at the end, because
	// mostly we are searching for view which was added to end in previous
	// iterations
	// if(((LayoutParams)getChildAt(j).getLayoutParams()).tileNumber ==
	// lp.tileNumber) {
	// arr[i] = null;
	// nullCounter++;
	// break;
	...
	protected void refillLeftSide() {
		...
		if (dspLeftScreenEdge >= dspNextViewsRight)
			return;
		// Logger.d(LOG_TAG, "from " + dspLeftScreenEdge + ", to " +
		// dspNextViewsRight);
	...
	private void removeNonVisibleViews() {
		...
		if (dspLeftScreenEdge <= 0)
			dspLeftScreenEdge--; // when values are <0 they get floored to value
									// which is larger
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			// if he had normal click in rested state, remember for action up
			// check
		...
		case MotionEvent.ACTION_UP:
			// if we had normal down click and we haven't moved enough to
			// initiate drag, take action as a click on down coordinates
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			// events will propagate back here and on intercept touch event wont
			// be called again
			// in case of no parent it propagates here, in case of parent it
			// usually propagates to on cancel
			if (mHandleSelectionOnActionUp && mTouchState == TOUCH_STATE_RESTING) {
				final float d = ToolBox.getLineLength(mDown.x, mDown.y, x, y);
	...
	/**
	 * In GRID_MODE_FIXED_SIZE mode has one dsp dimension set by setDspSize(),
	 * If band height is after transformation to normal pixels bigger than
	 * available space, content becomes scrollable also vertically.
	 * 
	 * In GRID_MODE_DYNAMIC_SIZE is dsp dimension computed from measured height
	 * and band height to always
	 */
	...
	/**
	 * Specifies how many normal pixels is in length of one device specific
	 * pixel This method is significant only in GRID_MODE_FIXED_SIZE mode (use
	 * setGridMode)
	 */
	...
	/**
	 * Set to true if you want component to work with tile z parameter; If you
	 * don't have any overlapping view, leave it on default false, because
	 * computing with z order makes rendering slower.
	 */
	...
	public interface Adapter {
		...
		/**
		 * Return Views which have left edge in device specific coordinates in
		 * range from-to,
		 * 
		 * @param from
		 *            inclusive
		...
		/**
		 * Return Views which have right edge in device specific coordinates in
		 * range from-to,
		 * 
		 * @param from
		 *            exclusive
		...
		/**
		 * Puts View, which is not needed anymore back to Adapter. View will be
		 * used later instead of creating or inflating same view.
		 */
	...
	public static abstract class AbstractAdapter<V extends View, Tile extends TileBase> implements Adapter {
		...
		/**
		 * Use getLayoutParamsForTile to get correct layout params for Tile data
		 * and set them with setLayoutParams before returning View
		 * 
		 * @param t
		 *            Tile data from datamodel
		 * @param recycled
		 *            View no more used and returned for recycling. Use together
		 *            with ViewHolder pattern to avoid performance loss in
		 *            inflating and searching by ids in more complex xml
		 *            layouts.
		 * @return View which will be displayed in component using layout data
		 *         from Tile
		 * 
		 *         <pre>
		 ...
		 * 		iw = recycled;
		 * 	else
		 * 		iw = new ImageView(MainActivity.this);
		 * 
		...
		public List<Tile> getTilesWithRightRange(int from, int to) {
			...
			final int fromIndex = binarySearchRightEdges(from + 1); // from is
																	// exclusive
		...
		/**
		 * Use this in getViewForTile implementation to provide correctly
		 * initialized layout params for component
		 * 
		 * @param t
		 *            Tile data from datamodel
	...
	private static class ViewCache<T extends View> {
		...
		/**
		 * Check if list of weak references has any view still in memory to
		 * offer for recycling
		 * 
		 * @return cached view
		 */
