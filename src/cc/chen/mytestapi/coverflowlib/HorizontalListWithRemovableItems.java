...
public class HorizontalListWithRemovableItems extends HorizontalList {
	...
	public HorizontalListWithRemovableItems(Context context, AttributeSet attrs, int defStyle) {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		if (isPointerDown) {
			if (ev.getActionMasked() == MotionEvent.ACTION_UP) {
				if (ToolBox.getLineLength(ev.getX(), ev.getY(), mDownX, mDownY) < mTouchSlop
	...
	//
	// if(mAlphaAnimationRunningOnIndex != -1) rightScreenEdge +=
	// mContainingView.getWidth();
	//
	...
	private Animator createRemoveAnimations(final int removedViewIndex) {
		...
		// anims.add(slider);
		// }
		// if(slider != null) slider.addUpdateListener(new
