...
public class StorageClient extends BaseActivity {
	public static final String FRAGTAG = "StorageClientFragment";

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.storage_client_main);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		getMenuInflater().inflate(R.menu.storage_client_menu, menu);
		return true;
	}
	...
	public class StorageClientFragment extends Fragment {
		...
		public boolean onOptionsItemSelected(MenuItem item) {
			if (item.getItemId() == R.id.storage_client_menu_item) {
				performFileSearch();
			}
			return true;
		}
		...
		/**
		 * Given the URI of an image, shows it on the screen using a
		 * DialogFragment.
		 * 
		 * @param uri
		...
		public void showImage(Uri uri) {
			...
			if (uri != null) {
				// Since the URI is to an image, create and show a
				// DialogFragment to display the
				// image to the user.
	...
	public static class ImageDialogFragment extends DialogFragment {
		...
		public Dialog onCreateDialog(Bundle savedInstanceState) {
			...
			// thread. Changing the ImageView to display the image must
			// occur ON the UI thread.
			// The easiest way to divide up this labor is with an AsyncTask.
			// The doInBackground
			// method will run in a separate thread, but onPostExecute will
			// run in the main
			// UI thread.
		...
		/**
		 * Grabs metadata for a document specified by URI, logs it to the
		 * screen.
		 * 
		 * @param uri
		...
		public void dumpImageMetaData(Uri uri) {
			...
			// The query, since it only applies to a single document, will
			// only return one row.
			// no need to filter, sort, or select fields, since we want all
			// fields for one
			// document.
			...
			try {
				...
				// Very handy for
				// "if there's anything to look at, look at it"
				// conditionals.
				if (cursor != null && cursor.moveToFirst()) {
					...
					// If the size is unknown, the value stored is null. But
					// since an int can't be
					// null in java, the behavior is
					// implementation-specific, which is just a fancy
					// term for "unpredictable". So as a rule, check if it's
					// null before assigning
					// to an int. This will happen often: The storage API
					// allows for remote
