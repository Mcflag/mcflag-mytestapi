...
/**
 * A descendant of {@link android.support.v4.widget.SwipeRefreshLayout} which
 * supports multiple child views triggering a refresh gesture. You set the views
 * which can trigger the gesture via {@link #setSwipeableChildren(int...)},
 * providing it the child ids.
 */
public class MultiSwipeRefreshLayout extends SwipeRefreshLayout {
	 ...
	 * 
	 * <p>
	 * As {@link android.support.v4.widget.SwipeRefreshLayout} only supports one
	...
	public boolean canChildScrollUp() {
		if (mSwipeableChildren != null && mSwipeableChildren.length > 0) {
			// Iterate through the scrollable children and check if any of them
			// can not scroll up
			for (View view : mSwipeableChildren) {
				if (view != null && view.isShown() && !canViewScrollUp(view)) {
					// If the view is shown, and can not scroll upwards, return
					// false and start the
					// gesture.
					return false;
				}
	...
	/**
	 * Utility method to check whether a {@link View} can scroll up from it's
