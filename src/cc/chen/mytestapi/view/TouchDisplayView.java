...
import cc.chen.mytestapi.utils.Pools.SimplePool;
 ...
 * use of {@link #onTouchEvent(android.view.MotionEvent)} and
 * {@link android.view.MotionEvent}s to keep track of touch pointers across
 * events.
 */
public class TouchDisplayView extends View {
	...
	static final class TouchHistory {
		...
		private static final SimplePool<TouchHistory> sPool = new SimplePool<TouchHistory>(MAX_POOL_SIZE);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN: {
			...
			/*
			 * Only one touch event is stored in the MotionEvent. Extract the
			 * pointer identifier of this touch from the first index within the
			 * MotionEvent object.
			 */
			...
			/*
			 * Store the data under its pointer identifier. The pointer number
			 * stays consistent for the duration of a gesture, accounting for
			 * other pointers going up or down.
			 */
		...
		case MotionEvent.ACTION_POINTER_DOWN: {
			 ...
			 * extract the index at which the data for this particular event is
			 * stored.
			 */
			int index = event.getActionIndex();
			int id = event.getPointerId(index);

			TouchHistory data = TouchHistory.obtain(event.getX(index), event.getY(index), event.getPressure(index));
			data.label = "id: " + id;

			/*
			 * Store the data under its pointer identifier. The index of this
			 * pointer can change over multiple events, but this pointer is
			 * always identified by the same identifier for this active gesture.
			 */
			mTouches.put(id, data);
		...
		case MotionEvent.ACTION_UP: {
			...
			/*
			 * Extract the pointer identifier for the only event stored in the
			 * MotionEvent object and remove it from the list of active touches.
			 */
			int id = event.getPointerId(0);
	...
	/**
	 * Sets up the required {@link android.graphics.Paint} objects for the
	 * screen density of this device.
	 */
	...
	/**
	 * Draws the data encapsulated by a {@link TouchDisplayView.TouchHistory}
	 * object to a canvas. A large circle indicates the current position held by
	 * the {@link TouchDisplayView.TouchHistory} object, while a smaller circle
	 * is drawn for each entry in its history. The size of the large circle is
	 * scaled depending on its pressure, clamped to a maximum of
	 * <code>1.0</code>.
	 * 
	 * @param canvas
	 * @param id
